...
# data cleaning 
text = re.sub("[^A-Za-z0-9.]", ' ', paragraph)   #removing all special characters
# text = re.sub(r"^\s+", "", text)
text = text.replace("  ", "")   #removing all white space in the text
text.replace(".", "")

#preparing the dataset
sentences = nltk.sent_tokenize(text)

#converting sentences to list of words to remove stopwords 
sentences = [nltk.word_tokenize(sentence) for sentence  in sentences ]
for i in range(len(sentences)): #loop to reove to stopwords
    sentences[i] = [word for word in sentences[i] if word not in set(stopwords.words("english"))]

#creating the word2 vec model
model =  Word2Vec(sentences, min_count  = 1)


#to see the worsd thta has been mapped 
words = model.wv.vocab

really = model.wv["really"] #tpo see the vector representation of a particular word
similar = model.wv.most_similar("really")    #To see the similar word to apartoiculatr word 
...
# text = re.sub(r'\s+', ' ', df_messages['messages'][i])
# text = text.lower()
